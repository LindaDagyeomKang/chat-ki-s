...
사내 문서 기반 RAG, OpenAI Function Calling 22개 도구, 자연어→SQL 변환(DataHub)을 결합하여
...
> 시각적 다이어그램: `docs/system-architecture-diagram.html` 참고
...
│             /api/documents                                   │
│  - AI연동:  /api/rag, /api/agent/execute, /api/datahub       │
...
│                                                              │
│  핵심 기능:                                                   │
│  - Function Calling: 22개 도구 중 적절한 도구 자동 선택       │
...
│  (port 8000)    │  │                                          │
│                 │  │  테이블 (12개 + 시스템 6개):              │
│  벡터 DB        │  │  - 사용자: users, employees (1,253명)    │
...
GPT-4o-mini가 22개 도구 중 적절한 도구 자동 선택
       │
       ├─────────────────┬──────────────────┐
       ▼                 ▼                  ▼
...
### 3-2. A. 정형 데이터 — Function Calling 도구 22개
...
| 검색 (6) | search_employees, search_documents, search_glossary, check_leave_status, find_substitute, check_room_availability | 임직원·사내문서·금융용어·휴가확인·대리인·회의실 검색 |
...
기존 22개 도구로 처리할 수 없는 집계/분석/통계 질문을 처리한다.
...
| AI Service (FastAPI) | Python, LangChain, OpenAI, ChromaDB | Function Calling 22개 도구, RAG, DataHub SQL 생성+1차 검증, Agent | 외부 직접 노출 금지 (백엔드 경유) |
...
│       │   ├── tools.py     # Function Calling 22개 도구 정의
...
│   └── seed_data/           # 임직원 프로필 1,253명, 금융용어 1,622개
