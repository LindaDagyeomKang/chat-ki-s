...
사용자 질문이 처리되는 전체 파이프라인은 4개 레이어를 통과한다.

(추후 png 추가 예정)

```
Layer 1: Intent Analysis    — 의도 분석 (Function Calling 도구 선택)
Layer 2: Knowledge Retrieval — 지식 검색 (RAG + DB + DataHub)
Layer 3: Reasoning & Generation — 추론 및 답변 생성 (GPT-4o-mini)
Layer 4: Validation & Filtering — 검증 및 필터링 (적절성 검증)
```

### 3-1. Layer 1 — Intent Analysis (의도 분석)

사용자 질문이 입력되면 GPT-4o-mini가 OpenAI Function Calling으로 23개 도구 중 적절한 것을 자동 선택한다. 이 단계에서 질문이 단순 조회인지, 문서 검색이 필요한지, 업무 실행 명령인지를 판단한다.

### 3-2. Layer 2 — Knowledge Retrieval (지식 검색, 3개 경로)
...
### 3-3. Layer 3 — Reasoning & Generation (추론 및 답변 생성)

- 검색된 지식(문서 청크, DB 조회 결과)을 LLM 컨텍스트에 주입
- GPT-4o-mini가 한국어 자연어로 답변 생성
- 하이브리드 검색(벡터 70% + 키워드 30%) 결과를 활용하여 검색 정확도 극대화

### 3-4. Layer 4 — Validation & Filtering (검증 및 필터링)

- 생성된 답변이 검색 문서에 근거하는지 LLM이 재검증 (프롬프트 체이닝)
- 유사도 ≥ 0.70 고신뢰 소스는 검증 생략 (신뢰 소스)
- 부적절 판정 시 폴백 응답으로 대체
- 출처 문서명 + 원본 다운로드 링크를 답변에 포함

### 3-5. Feedback Loop (피드백 수집)
