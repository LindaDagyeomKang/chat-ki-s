...
## 판단 기준
1. 답변이 검색된 문서의 내용과 명백히 모순되거나 완전히 지어낸 내용인가?
2. 검색된 문서의 내용을 요약하거나 재구성한 답변은 적절로 판정하세요.
3. 검색된 문서에 관련 내용이 있다면, 답변이 다소 일반적이더라도 적절로 판정하세요.
4. 명백한 거짓 정보(없는 제도, 없는 규정 등을 만들어낸 경우)만 부적절로 판정하세요.
...
async def _validate_answer(question: str, hits: list[dict], answer: str) -> bool:
    ...
    doc_summary = "\n".join(
        f"- {h['content'][:500]}... (출처: {h['source']})" for h in hits[:3]
    )
