...
export default function MailsPage() {
  ...
  async function handleBulkRestore() {
    for (const id of selectedIds) {
      try { await restoreMail(id) } catch {}
    }
    setSelectedIds(new Set())
    setSelected(null)
    fetchMails(activeFolder)
          ...
          <div className="flex items-center justify-between px-8 py-4" style={{ borderBottom: '1px solid #F3F4F5' }}>
            ...
            {selectedIds.size > 0 && (
              <div className="flex items-center gap-2">
                ...
                {activeFolder === 'trash' ? (
                  <>
                    <button onClick={handleBulkRestore} className="text-xs px-2 py-1 rounded-full hover:bg-green-50" style={{ color: '#10B981' }} title="복구">복구</button>
                    <button onClick={handleBulkDelete} className="text-xs px-2 py-1 rounded-full hover:bg-red-50" style={{ color: '#BA1A1A' }} title="완전 삭제">완전 삭제</button>
                  </>
                ) : (
                  <>
